<html>
  <head>
    <title>${Infotext.findByControllerAndAction('error','page_401')['title'+context?.lang]}</title>
    <meta name="description" content="${Infotext.findByControllerAndAction('error','page_401')['description'+context?.lang]?:''}" />
  ...
  <body>      
      <h1 role="heading">${Infotext.findByControllerAndAction('error','page_401')['header'+context?.lang]}</h1>
      <div data-role="content" style="padding:8px">        
        <g:formRemote name="loginForm" url="[action:'login']" onLoading="jQuery.mobile.showPageLoadingMsg('b','${message(code:'spinner.message.mobile')}',false)" onSuccess="processLogResponse(e)">
          <div class="notice" id="login_fail">            
            <g:rawHtml>${Infotext.findByControllerAndAction('error','page_401')['itext'+context?.lang].replace('<p>','').replace('</p>','')}</g:rawHtml>
          </div>
          ...
          <div class="ui-grid-a">
            <div class="ui-block-a"><input type="submit" data-theme="f" value="${message(code:'label.login')}" /></div>
