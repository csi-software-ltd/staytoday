<div id="ajax_wrap1">
  <div id="results_header" class="results_header">    
    <span class="count float" style="top:0px">
      <b>${message(code:'reviews.records.found')}</b><span id="ads_count">${myComments.count}</span>
    </span>
    <span class="pagination col">
      <g:paginate controller="profile" prev="&lt;&lt;" next="&gt;&gt;"
        action="${actionName}" max="20" params="${inrequest}"	total="${myComments.count}"/>
      <g:observe classes="${['step','prevLink','nextLink']}" event="click" function="clickPaginate"/>
    </span>
  </div>
<g:if test="${myComments.records}">
  <table class="list-results" width="100%" cellpadding="5" cellspacing="0" border="0">        
  <g:each in="${myComments.records}" status="i" var="record">                      
    <tr>  
      <td valign="top" onmouseout="this.removeClassName('selected');" onmouseover="this.addClassName('selected');">
        ...
        <div class="bubble-container col" style="width:585px;height:68px;overflow-y:auto">          
          <small>${record?.comtext}</small>                            
        </div>
        <div class="col" align="right" style="margin-right:10px">
          <g:if test="${record?.typeid==1}">
            ...
            <g:link class="stars_icon ${(record?.rating==2)?'positively':((record?.rating==1)?'negatively':'neutrally')}" mapping="${'pView'+context?.lang}" params="${[uid:'id'+record?.home_id]}">${message(code:'personal.anketa').capitalize()}</g:link>
          </g:else>          
          <g:if test="${record?.user_id==inrequest.u_id}">
            <a class="stars_icon none" href="javascript:void(0)" rel="nofollow" onclick="commentDelete(${record?.id},2);">${message(code:'reviews.delete')}</a>
          </g:if>
          </div>
        </div>
      </td>       
    </tr>
  <g:if test="${(i+1)!=myComments.records.size() || answerComments[i].size()>0}"> 
    ...
    <tr>
      <td valign="top" onmouseout="this.removeClassName('selected');" onmouseover="this.addClassName('selected');">
        <div class="float padd10" style="margin-left:84px">
          <div class="thumbnail userpic shadow">
            <img width="68" height="68" alt="${answerOwners[i]?.nickname}" title="${answerOwners[i]?.nickname}" 
              src="${(answerOwners[i]?.picture && !answerOwners[i]?.is_external)?imageurl:''}${(answerOwners[i]?.smallpicture)?answerOwners[i]?.smallpicture:resource(dir:'images',file:'user-default-picture.png')}">
          </div>
          <div style="width:68px">
            <small style="white-space:normal"><g:link mapping="${'pView'+context?.lang}" params="${[uid:'id'+answerOwners[i]?.id]}">${answerOwners[i]?.nickname}</g:link></small><br/>
            <small style="white-space:nowrap">${String.format('%td.%<tm.%<tY %<tH:%<tM',answ?.comdate)}</small>
          </div>
        </div>            
        <div class="bubble-container col" style="width:500px;height:68px;overflow-y:auto">          
          <small>${answ.comtext}</small>
        </div>
      </td>
    </tr>
    <g:if test="${(i+1)!=myComments.records.size() || (j+1)!=answerComments[i].size()}"> 
    <tr><td class="dashed">&nbsp;</td></tr>
    </g:if>
  </g:each>
</g:each>
