...
class MboxControlJob {
	...
	static triggers = {
		...
		cron cronExpression: ((Holders.config.mboxControl.cron!=[:])?Holders.config.mboxControl.cron:"0 55 * * * ?")
	}
	...
	def execute() {
		...
		def now = new Date()
		def lsMbox = Mbox.findAllByModdateLessThanAndControlstatus(new Date(now.getTime()-1000*60*Tools.getIntVal(Holders.config.noAnswer.strictCondition.minutes,720)),-1)
		lsMbox.each{
			try{
				it.controlstatus = -2
				it.modstatus = 2
				it.save(flush:true)
				if (oAdmin?.email)
					(new Zayavkabyemail(oAdmin.email,oAdmin.name,it.id.toString(),Tools.getIntVal(Holders.config.noAnswer.strictCondition.minutes,720).toString(),it.date_start,it.date_end,'#noAnswerNotice')).save(flush:true)
		...
		lsMbox = Mbox.findAllByModdateLessThanAndControlstatus(new Date(now.getTime()-1000*60*Tools.getIntVal(Holders.config.noAnswer.softCondition.minutes,120)),0)
		lsMbox.each{
			try{
				if (oAdmin?.email)
					(new Zayavkabyemail(oAdmin.email,oAdmin.name,it.id.toString(),Tools.getIntVal(Holders.config.noAnswer.softCondition.minutes,120).toString(),it.date_start,it.date_end,'#noAnswerNotice')).save(flush:true)
				it.controlstatus = -1
				it.save(flush:true)
			} catch (Exception e){
